...
#[cfg(all(target_arch = "x86_64", not(axvisor_host_riscv64)))]
...
#[cfg(any(target_arch = "riscv64", axvisor_host_riscv64))]
use riscv_vplic::VPlicGlobal;
#[cfg(all(target_arch = "x86_64", not(axvisor_host_riscv64)))]
use x86_vlapic::{
    EmulatedIoApic, EmulatedLocalApic, EmulatedPic, EmulatedPit, EmulatedSerialPort,
    IoApicEoi, IoApicInterrupt,
};
...
fn device_not_found<T>(
    ...
    ax_err!(
        Unsupported,
        format!("emu_device not found: {rw} {addr_type} {addr:#x} width {width:?}")
    )
}
...
impl AxVmDevices {
    ...
    pub fn new(config: AxVmDeviceConfig) -> Self {
        ...
        Self::init(&mut this, &config);
        this
    }

    /// According the emu_configs to init every  specific device
    fn init(this: &mut Self, device_config: &AxVmDeviceConfig) {
        for config in &device_config.emu_configs {
            match config.emu_type {
                ...
                EmulatedDeviceType::PPPTGlobal => {
                    ...
                    #[cfg(not(any(target_arch = "riscv64", axvisor_host_riscv64)))]
                    {
                        warn!(
                            "emu type: {} is not supported on this platform",
                            config.emu_type
                ...
                EmulatedDeviceType::Console => {
                    ...
                    #[cfg(any(not(target_arch = "x86_64"), axvisor_host_riscv64))]
                    {
                        warn!(
                            "emu type: {} is not supported on this platform",
                            config.emu_type
                ...
                EmulatedDeviceType::X86Pic => {
                    #[cfg(all(target_arch = "x86_64", not(axvisor_host_riscv64)))]
                    {
                        this.add_port_dev(Arc::new(EmulatedPic::new()));
                        info!("x86 PIC initialized for ports 0x20/0x21, 0xa0/0xa1, 0x4d0/0x4d1");
                    }
                    #[cfg(any(not(target_arch = "x86_64"), axvisor_host_riscv64))]
                    {
                        warn!(
                ...
                EmulatedDeviceType::X86LocalApic => {
                    #[cfg(all(target_arch = "x86_64", not(axvisor_host_riscv64)))]
                    {
                        const BSP_VCPU_ID: usize = 0;
                        this.add_mmio_dev(Arc::new(EmulatedLocalApic::new(
                            device_config.vm_id,
                            BSP_VCPU_ID,
                        )));
                        info!(
                            "x86 Local APIC initialized with base GPA {:#x} and length {:#x} for \
                             vm_id={} vcpu_id={}",
                            config.base_gpa, config.length, device_config.vm_id, BSP_VCPU_ID
                        );
                    }
                    #[cfg(any(not(target_arch = "x86_64"), axvisor_host_riscv64))]
                    {
    ...
    /// Replace one x86 IOAPIC redirection-table entry.
    #[cfg(all(target_arch = "x86_64", not(axvisor_host_riscv64)))]
    pub fn x86_ioapic_set_redirection_entry(&self, gsi: usize, entry: u64) -> bool {
        self.emu_mmio_devices.iter().any(|dev| {
            map_device_of_type(dev, |ioapic: &EmulatedIoApic| {
                ioapic.set_redirection_entry(gsi, entry)
            })
            .unwrap_or(false)
        })
    }
    ...
    /// Return the absolute wall-clock deadline (ns) of the x86 PIT channel 0's
    /// next IRQ0, if the PIT is armed. Used by the idle path to schedule a host
    /// wake-up so an idle guest keeps receiving periodic ticks.
    #[cfg(all(target_arch = "x86_64", not(axvisor_host_riscv64)))]
    pub fn x86_pit_next_irq0_deadline_ns(&self) -> Option<u64> {
        self.emu_port_devices.iter().find_map(|dev| {
            map_device_of_type(dev, |pit: &EmulatedPit| pit.next_irq0_deadline_ns()).flatten()
        })
    }

    /// Read from the x86 PIT port model without relying on broad port-range lookup.
    #[cfg(all(target_arch = "x86_64", not(axvisor_host_riscv64)))]
    pub fn x86_pit_handle_read(&self, port: Port, width: AccessWidth) -> AxResult<usize> {
        self.emu_port_devices
            .iter()
            .find_map(|dev| map_device_of_type(dev, |pit: &EmulatedPit| pit.handle_read(port, width)))
            .unwrap_or_else(|| device_not_found("port", port, true, width))
    }

    /// Write to the x86 PIT port model without relying on broad port-range lookup.
    #[cfg(all(target_arch = "x86_64", not(axvisor_host_riscv64)))]
    pub fn x86_pit_handle_write(&self, port: Port, width: AccessWidth, val: usize) -> AxResult {
        self.emu_port_devices
            .iter()
            .find_map(|dev| {
                map_device_of_type(dev, |pit: &EmulatedPit| pit.handle_write(port, width, val))
            })
            .unwrap_or_else(|| device_not_found("port", port, false, width))
    }

    /// Assert an x86 legacy PIC IRQ and return the vector to inject.
    #[cfg(all(target_arch = "x86_64", not(axvisor_host_riscv64)))]
    pub fn x86_pic_assert_irq(&self, irq: u8) -> Option<u8> {
        self.emu_port_devices.iter().find_map(|dev| {
            map_device_of_type(dev, |pic: &EmulatedPic| pic.assert_irq(irq)).flatten()
        })
    }

    /// Broadcast an x86 legacy PIC EOI.
    #[cfg(all(target_arch = "x86_64", not(axvisor_host_riscv64)))]
    pub fn x86_pic_end_of_interrupt(&self, vector: u8) {
        for dev in self.emu_port_devices.iter() {
            let _ = map_device_of_type(dev, |pic: &EmulatedPic| pic.end_of_interrupt(vector));
        }
    }

    /// Read from the x86 legacy PIC port model without relying on broad port-range lookup.
    #[cfg(all(target_arch = "x86_64", not(axvisor_host_riscv64)))]
    pub fn x86_pic_handle_read(&self, port: Port, width: AccessWidth) -> AxResult<usize> {
        self.emu_port_devices
            .iter()
            .find_map(|dev| map_device_of_type(dev, |pic: &EmulatedPic| pic.handle_read(port, width)))
            .unwrap_or_else(|| device_not_found("port", port, true, width))
    }

    /// Write to the x86 legacy PIC port model without relying on broad port-range lookup.
    #[cfg(all(target_arch = "x86_64", not(axvisor_host_riscv64)))]
    pub fn x86_pic_handle_write(&self, port: Port, width: AccessWidth, val: usize) -> AxResult {
        self.emu_port_devices
            .iter()
            .find_map(|dev| {
                map_device_of_type(dev, |pic: &EmulatedPic| pic.handle_write(port, width, val))
            })
            .unwrap_or_else(|| device_not_found("port", port, false, width))
    }
