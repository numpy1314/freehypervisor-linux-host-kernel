...
pub struct VirtualApicRegs {
    ...
    isrv: u32,
    /// Fallback vector for EOI exits when APICv has already consumed VISR.
    last_accepted_vector: u8,
...
impl VirtualApicRegs {
    ...
    fn find_isrv(&self) -> u32 {
        let mut isrv = 0;
        // ISR[0] contains vectors 0x00..0x1f. Valid external vectors start at
        // 0x10, so this slot still has to participate in EOI/PPR tracking.
        for i in (0..8).rev() {
    ...
    fn process_eoi(&mut self) -> Option<u8> {
        ...
        if vector == 0 {
            let fallback = self.last_accepted_vector;
            self.last_accepted_vector = 0;
            return (fallback != 0).then_some(fallback);
        }
        ...
        // If this EOI retires the periodic LAPIC timer interrupt, let the timer
        // inject the next tick. Under oversubscription the host hrtimer keeps
        // firing while the guest is behind; the drop-tick gate suppresses those
        // extra injections until this acknowledgement arrives (see
        // ApicTimerShared::tick_pending).
        if vector as u8 == self.virtual_timer.vector() {
            self.virtual_timer.ack_tick();
        }
    ...
    pub fn accept_interrupt(&mut self, vector: u8, level_triggered: bool) {
        ...
        self.isrv = self.find_isrv();
        self.last_accepted_vector = vector as u8;
        self.update_ppr();
    }
    ...
    fn calculate_dest_no_shorthand(
        ...
        if is_broadcast {
            ...
            for i in 0..vmm::current_vm_vcpu_num() {
                if vcpu_mask & (1 << i) != 0 {
                    let matched = if self.is_x2apic_enabled() {
                        dest == i as u32
                    } else {
                        match self
                            .regs()
                            ...
                            .ok_or(AxError::InvalidData)?
                        {
                            APICDestinationFormat::Flat => {
                                let dest_logical_id = dest & 0xff;
                                let target_logical_id = 1u32 << (i & 0x7);
                                target_logical_id & dest_logical_id != 0
                            }
                            APICDestinationFormat::Cluster => self.is_dest_field_matched(dest)?,
                        }
                    };
                    if !matched {
                        continue;
    ...
    fn process_init_sipi(
        ...
        match mode {
            APICDeliveryMode::INIT => {
                debug!(
                    "[VLAPIC] INIT IPI to vcpu {} icr_low={:#010X}",
                    vcpu_id,
                    icr_low.get()
                );
            }
            APICDeliveryMode::StartUp => {
                ...
                let entry_point = GuestPhysAddr::from((vector as usize) << 12);
                debug!(
                    "[VLAPIC] SIPI to vcpu {} vector={:#x} entry_point={:#x}",
                    vcpu_id,
                    vector,
                    entry_point.as_usize()
                );
                self.pending_cpu_up = Some(VlapicCpuUp {
                    target_cpu: vcpu_id as u64,
                    entry_point,
                });
            }
            _ => {}
    ...
    fn write_icr(&mut self) -> AxResult {
                ...
                || mode == APICDeliveryMode::StartUp)
        {
            ...
            // Sender-side store-buffer drain before publishing the IPI to the
            // target vCPU.
            //
            // This ICR write is a guest CALL_FUNCTION/RESCHEDULE IPI. On real
            // hardware the sender does `llist_add(&csd->node, remote cpu queue)`
            // and THEN writes the ICR; the APIC send (x2APIC WRMSR is
            // serializing, xAPIC MMIO send is ordered w.r.t. prior stores) makes
            // the queued CSD globally visible before the interrupt is delivered,
            // so the receiver's __flush_smp_call_function_queue() sees the entry.
            //
            // Here the guest's ICR write traps out to this host thread and we
            // publish the vector into the target vCPU's pending-interrupt state,
            // which the target's (different) host thread then injects. Nothing
            // guarantees the sending guest's prior `llist_add` store has drained
            // from this pCPU's store buffer to global visibility before we make
            // the IPI observable to the target. If it hasn't, the target vectors
            // into sysvec_call_function, ack_APIC_irq()s, drains an EMPTY queue,
            // and never runs the callback / csd_unlock — so the sender spins in
            // csd_lock_wait forever (observed on 2-vCPU Firecracker guests).
            //
            // Execute a real full fence (mfence) on this pCPU to drain the store
            // buffer BEFORE the IPI becomes observable to the target vCPU.
            // SAFETY: mfence has no operands and no memory/register side effects
            // beyond ordering; it is always valid to execute.
            unsafe {
...
impl VirtualApicRegs {
    ...
    /// (kvm_inject_apic_timer_irqs analog). See `ApicTimer::take_pending_tick`.
    pub fn take_pending_timer_tick(&self) -> Option<u8> {
        self.virtual_timer.take_pending_tick()
    }

    pub fn handle_apic_write_exit(&mut self, offset: usize) -> AxResult<Option<u8>> {
        let reg = match offset / 0x10 {
            0x0b => ApicRegOffset::EOI,
            0x0d => ApicRegOffset::LDR,
            0x0e => ApicRegOffset::DFR,
            0x0f => ApicRegOffset::SIVR,
        ...
        match reg {
            ApicRegOffset::EOI => Ok(self.process_eoi()),
            ApicRegOffset::ICRLow => {
                self.write_icr()?;
                Ok(None)
            }
