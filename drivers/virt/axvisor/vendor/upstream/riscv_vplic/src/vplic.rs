...
const PLIC_ENABLE_WORDS_PER_CONTEXT: usize = PLIC_NUM_SOURCES / 32;
...
pub struct VPlicGlobal {
    ...
    /// Virtual priority register values, indexed by IRQ source id.
    pub priorities: Mutex<[u32; PLIC_NUM_SOURCES]>,
    /// Virtual per-context enable words.
    pub enable_masks: Mutex<[[u32; PLIC_ENABLE_WORDS_PER_CONTEXT]; PLIC_MAX_CONTEXTS]>,
    /// Virtual per-context priority thresholds.
    pub thresholds: Mutex<[u32; PLIC_MAX_CONTEXTS]>,
...
impl VPlicGlobal {
    ...
    pub fn new(addr: GuestPhysAddr, size: Option<usize>, contexts_num: usize) -> Self {
        ...
        assert!(
            contexts_num <= PLIC_MAX_CONTEXTS,
            "context count {} exceeds virtual PLIC capacity {}",
            contexts_num,
            PLIC_MAX_CONTEXTS,
        );
        Self {
            ...
            enable_masks: Mutex::new([[0; PLIC_ENABLE_WORDS_PER_CONTEXT]; PLIC_MAX_CONTEXTS]),
            thresholds: Mutex::new([0; PLIC_MAX_CONTEXTS]),
            contexts_num,
