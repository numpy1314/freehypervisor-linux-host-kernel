...
impl<A: AxArchVCpu> AxVCpu<A> {
    ...
    where
        F: FnOnce() -> T,
    {
        if let Some(current) = get_current_vcpu::<A>() {
            axvisor_api::host::emerg_write_bytes(
                alloc::format!(
                    "axvcpu::nested current_vm={} current_vcpu={} target_vm={} target_vcpu={}\n",
                    current.vm_id(),
                    current.id(),
                    self.vm_id(),
                    self.id()
    ...
    /// Executes an operation on the architecture-specific vCPU without changing the vCPU state.
    pub fn with_arch_vcpu<F, T>(&self, f: F) -> AxResult<T>
    where
        F: FnOnce(&mut A) -> AxResult<T>,
    {
        self.with_current_cpu_set(|| f(self.get_arch_vcpu()))
