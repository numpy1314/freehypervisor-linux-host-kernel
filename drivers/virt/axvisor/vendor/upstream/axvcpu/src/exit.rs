...
pub enum AxVCpuExitReason {
    ...
    /// The guest executed a PAUSE-loop that exceeded the PLE window.
    ///
    /// This signals lock-holder-preemption avoidance: the current vCPU is
    /// spinning (typically waiting on a lock held by, or an AP being brought up
    /// by, another vCPU that the host scheduler has preempted). The VMM should
    /// directed-yield the physical CPU to another runnable-but-preempted vCPU
    /// and then resume this vCPU. This is handled entirely inside the host and
    /// never surfaces to a userspace VMM. It mirrors KVM's `kvm_vcpu_on_spin`
    /// (`handle_pause` -> `kvm_vcpu_yield_to` -> `yield_to`).
    Yield,
}
