...
pub struct VmxExitInfo {
    /// Raw VMCS exit-reason field, including high status bits.
    pub exit_reason_raw: u32,
...
pub fn exit_info() -> AxResult<VmxExitInfo> {
    ...
    Ok(VmxExitInfo {
        exit_reason_raw: full_reason,
        exit_reason: full_reason
            .get_bits(0..16)
            .try_into()
            .expect("Unknown VM-exit reason"),
        entry_failure: full_reason.get_bit(31),
        exit_qualification: VmcsReadOnlyNW::EXIT_QUALIFICATION.read()?,
        instruction_error_raw: VmcsReadOnly32::VM_INSTRUCTION_ERROR.read()?,
...
pub fn clear_injected_event() -> AxResult {
...
/// Read the IDT-vectoring information field for an event whose delivery was
/// interrupted by the current VM exit.
pub fn idt_vectoring_info() -> AxResult<VmxInterruptInfo> {
    // SDM Vol. 3C, Section 24.9.7
    let info = VmcsReadOnly32::IDT_VECTORING_INFO.read()?;
    let int_type = VmxInterruptionType::try_from(info.get_bits(8..11) as u8)
        .unwrap_or(VmxInterruptionType::Other);
    Ok(VmxInterruptInfo {
        vector: info.get_bits(0..8) as u8,
        int_type,
        err_code: if info.get_bit(11) {
...
pub fn inject_event_extint(vector: u8, err_code: Option<u32>, level_triggered: bool) -> AxResult {
    let err_code = if VmxInterruptionType::vector_has_error_code(vector) {
        err_code.or_else(|| Some(VmcsReadOnly32::VMEXIT_INTERRUPTION_ERR_CODE.read().unwrap()))
    } else {
        None
    };
    let mut int_info = VmxInterruptInfo::from(vector, err_code).bits();
    int_info.set_bit(15, level_triggered);
    if let Some(err_code) = err_code {
        VmcsControl32::VMENTRY_EXCEPTION_ERR_CODE.write(err_code)?;
    }
    VmcsControl32::VMENTRY_INTERRUPTION_INFO_FIELD.write(int_info)?;
    Ok(())
}
