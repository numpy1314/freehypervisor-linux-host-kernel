...
/// VMX MSR load/store entry. (SDM Vol. 3C, Section 24.7.2)
#[repr(C)]
...
impl VmxMsrEntry {
    pub const fn new(index: u32, data: u64) -> Self {
        Self {
            index,
            reserved: 0,
            data,
...
/// One page backing a VMX MSR load/store list.
#[derive(Debug)]
pub struct VmxMsrList {
    frame: PhysFrame,
}
...
impl VmxMsrList {
    ...
    pub fn write_entries(&mut self, entries: &[VmxMsrEntry]) -> AxResult {
        if entries.len() * size_of::<VmxMsrEntry>() > PAGE_SIZE {
            return ax_err!(NoMemory, "VMX MSR load/store list too large");
        }
        let dst = unsafe {
            core::slice::from_raw_parts_mut(
                self.frame.as_mut_ptr().cast::<VmxMsrEntry>(),
                entries.len(),
            )
        };
    ...
    pub fn read_entry(&self, index: usize) -> VmxMsrEntry {
        unsafe { *self.frame.as_mut_ptr().cast::<VmxMsrEntry>().add(index) }
    }
}
