...
pub trait VmmIf {
    ...
    /// Register a VMM timer owned by a specific target vCPU.
    ///
    /// This is equivalent to [`register_timer`] for backends whose timer
    /// placement is not vCPU-scoped. Backends with per-vCPU timer tables should
    /// use `target_vcpu_id` for table ownership instead of the caller's current
    /// run context.
    fn register_timer_on_vcpu(
        target_vm_id: VMId,
        target_vcpu_id: VCpuId,
        deadline: TimeValue,
        callback: Box<dyn FnOnce(TimeValue) + Send + 'static>,
    ) -> CancelToken;
    ...
    /// Handle a guest write to a KVM paravirtual clock MSR (x86 kvm-clock).
    ///
    /// This forwards `MSR_KVM_SYSTEM_TIME_NEW` (0x4b564d01) /
    /// `MSR_KVM_WALL_CLOCK_NEW` (0x4b564d00) writes to the host, which records
    /// the guest-physical address of the shared page and populates it. Without
    /// this the guest has no stable clocksource under `tsc=unstable` and its
    /// global jiffies freeze when the tick-owner vCPU is starved by
    /// oversubscription.
    ///
    /// Non-x86 backends implement this as a no-op.
    fn pvclock_write(vm_id: VMId, vcpu_id: VCpuId, msr: u32, value: u64);

    /// Refresh a vCPU's pvclock shared page so `system_time` keeps advancing.
    ///
    /// Called at KVM_RUN entry. Non-x86 backends implement this as a no-op.
    fn pvclock_refresh(vm_id: VMId, vcpu_id: VCpuId);
}
...
/// Get the vCPU count of the current VM.
pub fn current_vm_vcpu_num() -> usize {
    vm_vcpu_num(current_vm_id())
}

/// Get the active vCPU mask of the current VM.
pub fn current_vm_active_vcpus() -> usize {
    vm_active_vcpus(current_vm_id())
}
