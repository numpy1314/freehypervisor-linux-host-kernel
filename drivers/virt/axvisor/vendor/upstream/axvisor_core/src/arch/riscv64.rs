// Copyright 2025 The Axvisor Team
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

use axaddrspace::{GuestPhysAddr, device::AccessWidth};
use axvm::config::VMInterruptMode;

use crate::vmm::VM;
use crate::vmm::vm_list::get_vm_by_id;

const GUEST_PLIC_PADDR: usize = 0x0c00_0000;

pub fn hardware_check() {
    // TODO: implement hardware checks for RISC-V64
    // check page table level like aarch64
}

pub fn inject_current_interrupt(irq_id: usize) -> bool {
    let Some(context) = crate::context::try_current_vcpu_context() else {
        return false;
    };
    inject_interrupt(context.vm_id, irq_id)
}

pub fn inject_interrupt(vm_id: usize, irq_id: usize) -> bool {
    debug!("injecting interrupt id: {}", irq_id);
    info!("riscv64::inject_interrupt vm_id={vm_id} irq_id={irq_id}");

    let vm = get_vm_by_id(vm_id).unwrap();
    let effective_irq_id = translated_guest_irq_id(&vm, irq_id);

    if effective_irq_id == 0 {
        warn!("riscv64::inject_interrupt vm_id={vm_id} resolved irq_id=0, skip injection");
        return false;
    }

    let vplic = vm
        .get_devices()
        .find_mmio_dev(GuestPhysAddr::from_usize(GUEST_PLIC_PADDR))
        .unwrap();

    let reg_offset = riscv_vplic::PLIC_PENDING_OFFSET + (effective_irq_id / 32) * 4;
    let addr = GuestPhysAddr::from_usize(GUEST_PLIC_PADDR + reg_offset);
    let width = AccessWidth::Dword;
    let val: u32 = 1 << (effective_irq_id % 32);

    if let Err(err) = vplic.handle_write(addr, width, val as _) {
        warn!("failed to inject interrupt id {effective_irq_id} into guest vPLIC: {err:?}");
        return false;
    }
    true
}

fn translated_guest_irq_id(vm: &VM, fallback_irq_id: usize) -> usize {
    vm.with_config(|cfg| {
        if cfg.interrupt_mode() != VMInterruptMode::Passthrough {
            return fallback_irq_id;
        }

        if fallback_irq_id != 0
            && cfg
            .pass_through_devices()
            .iter()
            .any(|dev| dev.irq_id == fallback_irq_id)
        {
            fallback_irq_id
        } else {
            warn!(
                "riscv64::inject_interrupt drop unmatched passthrough irq vm_id={} host_irq_id={}",
                vm.id(),
                fallback_irq_id
            );
            0
        }
    })
}
