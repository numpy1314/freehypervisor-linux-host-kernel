...
pub fn console_init() {
    info!("shell: console_init start");
    crate::println!("shell::console_init entered");
    host_emerg("shell::console_init entered");
    ...
    loop {
        ...
        let ch = match stdin.read(&mut temp_buf) {
            Ok(1) => {
                info!("shell: read byte 0x{:02x}", temp_buf[0]);
                crate::println!("shell::read byte=0x{:02x}", temp_buf[0]);
                host_emerg("shell::read byte");
                temp_buf[0]
            }
            _ => {
                continue;
                ...
                match ch {
                    CR | LF => {
                        ...
                        if line_len > 0 {
                            ...
                            crate::println!("shell::command line='{}'", cmd_str);
                            host_emerg("shell::command line");
                            ...
                            if !handle_builtin_commands(cmd_str) {
                                crate::println!("shell::run_cmd_bytes before");
                                host_emerg("shell::run_cmd_bytes before");
                                run_cmd_bytes(&buf[..line_len]);
                                crate::println!("shell::run_cmd_bytes after");
                                host_emerg("shell::run_cmd_bytes after");
                            }
