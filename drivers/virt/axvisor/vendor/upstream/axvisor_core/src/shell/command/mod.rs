...
pub static COMMAND_TREE: LazyInit<BTreeMap<String, CommandNode>> = LazyInit::new();

fn command_tree() -> &'static BTreeMap<String, CommandNode> {
    COMMAND_TREE.get_or_init(build_command_tree)
}
...
pub fn execute_command(input: &str) -> Result<(), ParseError> {
    host_emerg("command::execute_command enter");
    let parsed = CommandParser::parse(input)?;
    host_emerg("command::execute_command parsed");
    ...
    if let Some(handler) = current_node.handler {
        host_emerg("command::execute_command before_handler");
        handler(&parsed);
        host_emerg("command::execute_command after_handler");
        Ok(())
    } else {
...
pub fn prompt_string() -> String {
    #[cfg(feature = "fs")]
    {
        match axvisor_api::fs::current_dir() {
