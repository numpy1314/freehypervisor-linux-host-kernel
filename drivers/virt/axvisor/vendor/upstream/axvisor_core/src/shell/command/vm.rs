...
fn host_emerg(msg: &str) {
    host::emerg_write_bytes(msg.as_bytes());
}
...
fn vm_create(cmd: &ParsedCommand) {
    ...
    for config_path in args.iter() {
        ...
        match axvisor_api::fs::read_to_string(config_path) {
            Ok(raw_cfg) => {
                ...
                match init_guest_vm(&raw_cfg) {
                    Ok(vm_id) => {
                        ...
                        info!(
                            "vm_create: init_guest_vm ok config_path={} vm_id={}",
                            config_path, vm_id
                        );
                        println!(
                    ...
                    Err(e) => {
                        error!("Failed to create VM from {}: {:?}", config_path, e);
                        println!(
            ...
            Err(e) => {
                error!("vm_create: read_to_string failed config_path={} err={:?}", config_path, e);
    ...
    if created_count > 0 {
        ...
        println!("Created VM IDs: {:?}", created_vm_ids);
...
fn vm_start(cmd: &ParsedCommand) {
    host_emerg("vm_start::handler enter");
    let args = &cmd.positional_args;
    ...
    if args.is_empty() {
        ...
        for vm_name in args {
            ...
            if let Ok(vm_id) = vm_name.parse::<usize>() {
                host_emerg("vm_start::handler parsed_vm_id");
...
fn start_single_vm(vm: crate::vmm::VMRef) -> Result<(), &'static str> {
    host_emerg("vm_start::start_single_vm enter");
    let vm_id = vm.id();
    let status = vm.vm_status();
    host_emerg("vm_start::start_single_vm before_can_start");
    info!("vm_start: start_single_vm begin vm_id={} status={:?}", vm_id, status);
    crate::println!("vm_start::start_single_vm begin vm_id={}", vm_id);

    // Validate state transition using helper function
    can_start_vm(status)?;
    host_emerg("vm_start::start_single_vm after_can_start");

    // Set up primary virtual CPU before starting
    host_emerg("vm_start::start_single_vm before_setup_primary_vcpu");
    info!("vm_start: setup primary vcpu vm_id={}", vm_id);
    crate::println!("vm_start::setup_primary_vcpu vm_id={}", vm_id);
    vcpus::setup_vm_primary_vcpu(vm.clone());
    host_emerg("vm_start::start_single_vm after_setup_primary_vcpu");

    // Boot the VM
    host_emerg("vm_start::start_single_vm before_boot");
    crate::println!("vm_start::before_boot vm_id={}", vm_id);
    match vm.boot() {
        Ok(_) => {
            host_emerg("vm_start::start_single_vm after_boot");
            info!("vm_start: boot ok vm_id={}", vm_id);
            crate::println!("vm_start::after_boot vm_id={}", vm_id);
            ...
            info!("vm_start: notify primary vcpu vm_id={}", vm_id);
            host_emerg("vm_start::start_single_vm before_notify_primary_vcpu");
            crate::println!("vm_start::before_notify_primary_vcpu vm_id={}", vm_id);
            vcpus::notify_primary_vcpu(vm_id);
            host_emerg("vm_start::start_single_vm after_notify_primary_vcpu");
            crate::println!("vm_start::after_notify_primary_vcpu vm_id={}", vm_id);
            info!("vm_start: add running count vm_id={}", vm_id);
            add_running_vm_count(1);
            info!("vm_start: start_single_vm success vm_id={}", vm_id);
            host_emerg("vm_start::start_single_vm success");
            crate::println!("vm_start::success vm_id={}", vm_id);
            Ok(())
        }
        Err(err) => {
            // Revert status on failure
            error!("Failed to boot VM[{}]: {:?}", vm_id, err);
            host_emerg("vm_start::start_single_vm boot_err");
            crate::println!("vm_start::boot_err vm_id={}", vm_id);
...
fn start_vm_by_id(vm_id: usize) {
    host_emerg("vm_start::start_vm_by_id enter");
    info!("vm_start: requested vm_id={}", vm_id);
    match with_vm(vm_id, |vm| start_single_vm(vm.clone())) {
        Some(Ok(_)) => {
            info!("vm_start: vm_id={} start_single_vm ok", vm_id);
            println!("✓ VM[{}] started successfully", vm_id);
        }
        Some(Err(err)) => {
            warn!("vm_start: vm_id={} start_single_vm err={}", vm_id, err);
            println!("✗ VM[{}] failed to start: {}", vm_id, err);
        }
        None => {
            warn!("vm_start: vm_id={} not found", vm_id);
...
fn show_vm_full_details(vm_id: usize) {
    match with_vm(vm_id, |vm| {
        ...
        for (i, region) in vm.memory_regions().iter().enumerate() {
            let region_type = if region.is_allocated() {
                "Allocated"
            } else {
                "Reserved"
            };
...
pub fn build_vm_cmd(tree: &mut BTreeMap<String, CommandNode>) {
    ...
    #[cfg(feature = "fs")]
    {
        vm_node = vm_node.add_subcommand("create", create_cmd);
    }
