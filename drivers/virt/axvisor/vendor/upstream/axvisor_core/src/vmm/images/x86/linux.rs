...
pub const HEADER_READ_SIZE: usize = INIT_SIZE_OFFSET + core::mem::size_of::<u32>();
...
impl X86LinuxHeader {
    pub fn parse(image: &[u8]) -> Result<Self, X86LinuxHeaderError> {
        ...
        Ok(Self {
            ...
            init_size: read_u32(image, INIT_SIZE_OFFSET)?,
        })
    }
...
mod tests {
    ...
    const INIT_SIZE: u32 = 0x02db_e000;
    ...
    fn valid_bzimage_header() -> [u8; HEADER_READ_SIZE] {
        ...
        write_u32(&mut image, INIT_SIZE_OFFSET, INIT_SIZE);
        image
    }
    ...
    fn parses_valid_bzimage_header() {
        ...
        assert_eq!(header.init_size, INIT_SIZE);
