...
pub fn init() {
    crate::println!("vmm::init stage=enter");
    info!("Initializing VMM...");
    VMM.init_once(axvisor_api::sync::WaitQueue::new());
    crate::println!("vmm::init stage=waitqueue_ready");
    // Initialize guest VM according to config file.
    crate::println!("vmm::init stage=before_init_guest_vms");
    config::init_guest_vms();
    crate::println!("vmm::init stage=after_init_guest_vms");

    // Setup vCPUs, spawn an ax-task for the primary vCPU.
    crate::println!("vmm::init stage=before_setup_vcpus");
    info!("Setting up vcpus...");
    for vm in vm_list::get_vm_list() {
        crate::println!("vmm::init stage=setup_primary_vcpu vm_id={}", vm.id());
        vcpus::setup_vm_primary_vcpu(vm);
    }
    crate::println!("vmm::init stage=after_setup_vcpus");

    #[cfg(any(target_arch = "x86_64", target_arch = "riscv64", axvisor_host_riscv64))]
    release_host_resources_for_guest_passthrough().expect(
        "Failed to release host resources before guest passthrough devices take ownership",
    );
    crate::println!("vmm::init stage=leave");
}

#[cfg(any(target_arch = "x86_64", target_arch = "riscv64", axvisor_host_riscv64))]
fn release_host_resources_for_guest_passthrough() -> AxResult {
    ...
    info!("Host resources released before guest passthrough devices start");
    Ok(())
}
...
pub fn with_vm_and_vcpu_on_pcpu(
    ...
    if let Some(context) = crate::context::try_current_vcpu_context() {
        if context.vm_id == vm_id && context.vcpu_id == vcpu_id {
...
impl api_vmm::VmmIf for VmmIfImpl {
    fn current_vm_id() -> VMId {
        crate::context::current_vm_id()
    }

    fn current_vcpu_id() -> VCpuId {
        crate::context::current_vcpu_id()
    }
    ...
    fn inject_interrupt(vm_id: VMId, vcpu_id: VCpuId, vector: InterruptVector) {
        if let Err(err) = vcpus::queue_interrupt(vm_id, vcpu_id, vector as usize) {
            warn!(
                "Failed to queue interrupt {:#x} for VM[{}] VCpu[{}]: {:?}",
                vector, vm_id, vcpu_id, err
            );
        }
    }
    ...
    fn pvclock_write(_vm_id: VMId, _vcpu_id: VCpuId, _msr: u32, _value: u64) {
        // pvclock (x86 kvm-clock) is only implemented by the Linux KVM x86
        // bridge; the generic core has no host page to populate.
    }

    fn pvclock_refresh(_vm_id: VMId, _vcpu_id: VCpuId) {}
}
