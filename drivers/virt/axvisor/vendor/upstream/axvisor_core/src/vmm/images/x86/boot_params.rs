...
impl<'a> BootParamsBuilder<'a> {
    ...
    pub fn build(mut self) -> Result<Vec<u8>, BootParamsError> {
        if let Some(initrd) = self.layout.initrd {
            self.add_reserved_range(initrd);
        }

        let mut boot_params = alloc::vec![0u8; BOOT_PARAMS_SIZE];
...
mod tests {
    ...
    #[test]
    fn records_initrd_as_reserved_in_e820() {
        let image = valid_image();
        let header = X86LinuxHeader::parse(&image).unwrap();
        let layout = valid_layout(&header);
        let params =
        ...
        let entries = read_u8(&params, E820_ENTRIES_OFFSET) as usize;
        let found_reserved_initrd = (0..entries).any(|idx| {
            read_e820_entry(&params, idx)
                == E820Entry::reserved(X86LinuxRange::new(0x40_0000, 0x20_0000)).unwrap()
        });
        let found_usable_initrd = (0..entries).any(|idx| {
            let entry = read_e820_entry(&params, idx);
            entry.entry_type == E820_TYPE_RAM
                && entry.addr < 0x60_0000
                && entry.addr + entry.size > 0x40_0000
        });

        assert!(found_reserved_initrd);
        assert!(!found_usable_initrd);
    }
