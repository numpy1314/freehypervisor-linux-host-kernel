...
pub fn build(phys_cpu_ids: &[usize]) -> Vec<u8> {
    let mut image = alloc::vec![0u8; MP_TABLE_SIZE];
    let config = build_config_table(phys_cpu_ids);
...
fn build_config_table(phys_cpu_ids: &[usize]) -> Vec<u8> {
    let entries = config_entries(phys_cpu_ids);
...
fn config_entries(phys_cpu_ids: &[usize]) -> Vec<Vec<u8>> {
    let cpu_ids = if phys_cpu_ids.is_empty() {
        &[BSP_APIC_ID as usize][..]
    } else {
        phys_cpu_ids
    };

    let mut entries = Vec::with_capacity(cpu_ids.len() + 3 + 32);
    for (index, phys_cpu_id) in cpu_ids.iter().enumerate() {
        entries.push(processor_entry(*phys_cpu_id as u8, index == 0));
    }
...
fn processor_entry(apic_id: u8, is_bsp: bool) -> Vec<u8> {
    let mut entry = Vec::with_capacity(20);
    entry.push(0);
    entry.push(apic_id);
    entry.push(APIC_VERSION);
    entry.push(if is_bsp { 0x03 } else { 0x01 }); // enabled, plus BSP for CPU0
...
mod tests {
    ...
    #[test]
    fn emits_one_processor_entry_per_configured_cpu() {
        let image = build(&[0, 1]);
        let entry_count = u16::from_le_bytes([image[34], image[35]]) as usize;
        assert_eq!(entry_count, 2 + 2 + 1 + 16 + 16);

        assert_eq!(image[44], 0);
        assert_eq!(image[45], 0);
        assert_eq!(image[47], 0x03);
        assert_eq!(image[64], 0);
        assert_eq!(image[65], 1);
        assert_eq!(image[67], 0x01);
    }
