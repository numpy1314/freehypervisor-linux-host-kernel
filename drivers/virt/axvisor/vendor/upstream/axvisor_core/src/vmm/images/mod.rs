    ...
    vmcfg::get_memory_images()
        ...
        .ok_or_else(|| {
            #[cfg(axvisor_host_riscv64)]
            {
                ax_err_type!(
                    NotFound,
                    "VM images are missing: Linux Kbuild does not run axvisor_core/build.rs, \
                     so AXVISOR_VM_CONFIGS cannot populate image_location=\"memory\". \
                     Use image_location=\"fs\" and place kernel/dtb under /guest."
                )
            }
...
impl ImageLoader {
    ...
    pub fn load(&mut self) -> AxResult {
        ...
        info!(
            "Loading VM[{}] images into memory region: gpa={:#x}, hva={:#x}, size={} bytes",
            self.vm.id(),
            self.main_memory.gpa,
            self.main_memory.hva,
            self.main_memory.size()
        );
    ...
    fn load_x86_linux_images_from_memory(
        ...
        crate::println!(
            "x86_linux_memory: enter vm_id={} kernel_len={} ramdisk_len={}",
            self.vm.id(),
            kernel.len(),
            ramdisk.map(|r| r.len()).unwrap_or(0)
        );
        self.relocate_x86_linux_direct_boot_layout(&header, kernel, ramdisk.map(|r| r.len()))?;
        crate::println!(
            "x86_linux_memory: after_relocate kernel_gpa={:#x} ramdisk_gpa={:?}",
            self.kernel_load_gpa.as_usize(),
            self.ramdisk_load_gpa
        );
        let payload = x86_linux_payload(&header, kernel)?;
        crate::println!(
            "x86_linux_memory: payload offset={:#x} len={}",
        ...
        crate::println!(
            "x86_linux_memory: layout boot_params={:#x}/{} boot_stub={:#x}/{} kernel={:#x}/{} initrd={:?}",
            layout.boot_params.start,
            layout.boot_params.size,
            layout.boot_stub.start,
            layout.boot_stub.size,
            layout.kernel.start,
            layout.kernel.size,
            layout.initrd
        );

        crate::println!("x86_linux_memory: before_load_layout");
        self.load_x86_linux_layout(header, layout, kernel)?;
        crate::println!("x86_linux_memory: after_load_layout");
        crate::println!("x86_linux_memory: before_load_payload");
        load_vm_image_from_memory(payload, self.kernel_load_gpa, self.vm.clone())?;
        crate::println!("x86_linux_memory: after_load_payload");

        if let Some(buffer) = ramdisk {
            crate::println!("x86_linux_memory: before_load_ramdisk");
            self.load_ramdisk_from_memory(buffer)?;
            crate::println!("x86_linux_memory: after_load_ramdisk");
        }
    ...
    fn load_ramdisk_from_memory(&self, ramdisk: &[u8]) -> AxResult {
        ...
        crate::println!(
            "ramdisk_memory: gpa={:#x} size={} first={:02x?} last={:02x?} checksum={:#x}",
            load_gpa.as_usize(),
            size,
            &ramdisk[..size.min(8)],
            &ramdisk[size.saturating_sub(size.min(8))..],
            image_byte_checksum(ramdisk)
        );
    ...
    fn relocate_x86_linux_direct_boot_layout(
        ...
        let memory_start = self.main_memory.gpa.as_usize();
        let memory_end = memory_start
            .checked_add(self.main_memory.size())
            .ok_or_else(|| ax_errno::ax_err_type!(InvalidInput, "guest RAM range overflows"))?;
        let kernel_load_gpa = if self.main_memory.is_identical() {
            memory_start
                .checked_add(self.config.kernel.kernel_load_addr)
                .ok_or_else(|| {
                    ax_errno::ax_err_type!(InvalidInput, "x86 Linux kernel load GPA overflows")
                })?
        } else {
            self.kernel_load_gpa.as_usize()
        };
        ...
        let decompressed_size = if header.init_size == 0 {
            payload_size
        } else {
            header.init_size as usize
        };
        let Some(kernel_scratch_end) =
            kernel_load_gpa.checked_add(payload_size.max(decompressed_size))
        else {
            return Ok(());
        };
        ...
        let relocated_ramdisk = if let Some(ramdisk_size) = ramdisk_size {
            let configured_ramdisk_gpa = self
                .config
                .kernel
                .ramdisk_load_addr
                .map(|ramdisk_load_addr| {
                    if self.main_memory.is_identical() {
                        memory_start.checked_add(ramdisk_load_addr)
                    } else {
                        Some(ramdisk_load_addr)
                    }
                })
                .flatten();
            let ramdisk_load_gpa = x86_direct_boot_ramdisk_gpa(
                configured_ramdisk_gpa,
                kernel_load_gpa,
                kernel_scratch_end,
                ramdisk_size,
            )
            .ok_or_else(|| {
                ax_errno::ax_err_type!(
                    InvalidInput,
                    "failed to place x86 Linux initrd without overlapping kernel"
                )
            })?;
            let ramdisk_end = ramdisk_load_gpa.checked_add(ramdisk_size).ok_or_else(|| {
                ax_errno::ax_err_type!(InvalidInput, "x86 Linux initrd range overflows")
            })?;
            if ramdisk_end > max_load_end || ramdisk_end > memory_end {
                info!(
                    "Keeping x86 Linux direct boot low-GPA layout for VM[{}]: relocated initrd \
                     end {:#x} exceeds initrd_addr_max {:#x} or RAM end {:#x}",
                    self.vm.id(),
                    ramdisk_end,
                    header.initrd_addr_max,
                    memory_end
    ...
    fn load_x86_linux_layout(
        ...
        crate::println!("x86_linux_layout: before_build_boot_params");
        let boot_params = self.build_x86_boot_params(header, layout, kernel)?;
        crate::println!("x86_linux_layout: after_build_boot_params len={}", boot_params.len());
        crate::println!("x86_linux_layout: before_build_boot_stub");
        let boot_stub = self.build_x86_linux_boot_stub(&layout)?;
        crate::println!("x86_linux_layout: after_build_boot_stub len={}", boot_stub.len());
        crate::println!("x86_linux_layout: before_build_mp_table");
        let phys_cpu_ids = self.config.base.phys_cpu_ids.clone().unwrap_or_else(|| {
            (0..self.config.base.cpu_num).collect::<alloc::vec::Vec<_>>()
        });
        let mp_table = x86_mptable::build(&phys_cpu_ids);
        crate::println!("x86_linux_layout: after_build_mp_table len={}", mp_table.len());
        crate::println!("x86_linux_layout: before_load_boot_params");
        load_vm_image_from_memory(
            &boot_params,
            layout.boot_params.start.into(),
            self.vm.clone(),
        )?;
        crate::println!("x86_linux_layout: after_load_boot_params");
        crate::println!("x86_linux_layout: before_load_boot_stub");
        load_vm_image_from_memory(&boot_stub, layout.boot_stub.start.into(), self.vm.clone())?;
        crate::println!("x86_linux_layout: after_load_boot_stub");
        crate::println!("x86_linux_layout: before_load_mp_table");
        load_vm_image_from_memory(&mp_table, x86_mptable::MP_TABLE_GPA.into(), self.vm.clone())?;
        crate::println!("x86_linux_layout: after_load_mp_table");
...
#[cfg(target_arch = "x86_64")]
fn x86_direct_boot_ramdisk_gpa(
    configured_gpa: Option<usize>,
    kernel_start: usize,
    kernel_end: usize,
    ramdisk_size: usize,
) -> Option<usize> {
    ...
    if ramdisk_size == 0 {
        return configured_gpa;
    }

    if let Some(configured_gpa) = configured_gpa {
        let configured_end = configured_gpa.checked_add(ramdisk_size)?;
        if configured_end <= kernel_start || configured_gpa >= kernel_end {
            return Some(configured_gpa);
        }
    }
...
mod tests {
    use super::x86_direct_boot_ramdisk_gpa;

    #[test]
    fn x86_ramdisk_relocates_when_configured_addr_overlaps_kernel_scratch() {
        assert_eq!(
            x86_direct_boot_ramdisk_gpa(
                Some(0x0105_6000),
                0x0020_0000,
                0x02fb_e000,
                0x0010_9000,
    ...
    #[test]
    fn x86_ramdisk_keeps_configured_addr_outside_kernel_scratch() {
        assert_eq!(
            x86_direct_boot_ramdisk_gpa(
                Some(0x0400_0000),
                0x0020_0000,
...
pub fn load_vm_image_from_memory(
    ...
    crate::println!(
        "image_load_memory: gpa={:#x} size={} first={:02x?} last={:02x?} checksum={:#x}",
        load_addr.as_usize(),
        image_size,
        &image_buffer[..image_size.min(8)],
        &image_buffer[image_size.saturating_sub(image_size.min(8))..],
        image_byte_checksum(image_buffer)
    );
    ...
    for region in image_load_regions {
        ...
        if region[..bytes_to_write] != image_buffer[buffer_pos..buffer_pos + bytes_to_write] {
            return ax_err!(
                InvalidData,
                format!(
                    "VM image memory verification failed at GPA {:#x} chunk_offset={} \
                     chunk_size={}",
                    load_addr.as_usize(),
                    buffer_pos,
                    bytes_to_write
...
fn image_byte_checksum(image_buffer: &[u8]) -> u64 {
    image_buffer
        .iter()
...
pub mod fs {
    ...
    pub fn kernel_read(config: &AxVMCrateConfig, read_size: usize) -> AxResult<Vec<u8>> {
        ...
        crate::println!("images::fs::kernel_read: open path={} read_size={}", file_name, read_size);
        ...
        crate::println!("images::fs::kernel_read: open ok path={}", file_name);
        ...
        crate::println!("images::fs::kernel_read: read_exact ok path={} read_size={}", file_name, read_size);
    ...
    pub(crate) fn load_vm_images_from_filesystem(loader: &mut ImageLoader) -> AxResult {
        crate::println!(
            "images::fs::load_vm_images_from_filesystem: enter vm_id={} kernel_path={} kernel_gpa={:#x}",
        ...
        // Load kernel image.
        crate::println!(
            "images::fs::load_vm_images_from_filesystem: before kernel load path={} gpa={:#x}",
            loader.config.kernel.kernel_path,
            loader.kernel_load_gpa.as_usize()
        );
        load_vm_image(
        ...
        crate::println!(
            "images::fs::load_vm_images_from_filesystem: after kernel load path={}",
            loader.config.kernel.kernel_path
        );
        // Load boot firmware image if needed.
        if loader.config.kernel.enable_bios {
            if let Some(bios_path) = loader.config.kernel.boot_firmware_path() {
        ...
        if let Some(ramdisk_path) = &loader.config.kernel.ramdisk_path {
            crate::println!(
                "images::fs::load_vm_images_from_filesystem: before ramdisk load path={}",
                ramdisk_path
            );
            loader.load_ramdisk_from_filesystem(ramdisk_path)?;
            crate::println!(
                "images::fs::load_vm_images_from_filesystem: after ramdisk load path={}",
                ramdisk_path
            );
        };
        ...
        crate::println!(
            "images::fs::load_vm_images_from_filesystem: leave vm_id={}",
            loader.config.base.id
        );
        Ok(())
    }
    ...
    pub(crate) fn load_vm_image(
        ...
        crate::println!(
            "images::fs::load_vm_image: enter path={} gpa={:#x}",
            image_path,
            image_load_gpa.as_usize()
        );
        let (image_file, image_size) = open_image_file(image_path)?;
        crate::println!(
            "images::fs::load_vm_image: open ok path={} size={}",
            image_path,
            image_size
        );

        crate::println!(
            "images::fs::load_vm_image: before get_image_load_region path={} gpa={:#x} size={}",
            image_path,
            image_load_gpa.as_usize(),
            image_size
        );
        ...
        crate::println!(
            "images::fs::load_vm_image: after get_image_load_region path={} size={}",
            image_path,
            image_size
        );
        let mut file = image_file;

        const LINUX_HOST_IMAGE_READ_STEP: usize = 256 * 1024;

        for (idx, buffer) in image_load_regions.into_iter().enumerate() {
            crate::println!(
                "images::fs::load_vm_image: before read_exact path={} chunk={} len={} dst={:#x}",
                image_path,
                idx,
                buffer.len(),
                buffer.as_ptr() as usize
            );
            for (sub_idx, sub_buf) in buffer.chunks_mut(LINUX_HOST_IMAGE_READ_STEP).enumerate() {
                crate::println!(
                    "images::fs::load_vm_image: before read_exact_sub path={} chunk={} sub={} len={} dst={:#x}",
                    image_path,
                    idx,
                    sub_idx,
                    sub_buf.len(),
                    sub_buf.as_ptr() as usize
                );
                file.read_exact(sub_buf).map_err(|err| {
                    ax_err_type!(
                        Io,
                ...
                crate::println!(
                    "images::fs::load_vm_image: after read_exact_sub path={} chunk={} sub={}",
                    image_path,
                    idx,
                    sub_idx
                );
            ...
            crate::println!(
                "images::fs::load_vm_image: after read_exact path={} chunk={}",
                image_path,
                idx
            );
        ...
        crate::println!("images::fs::load_vm_image: leave path={}", image_path);
        Ok(())
    }
    ...
    pub fn open_image_file(file_name: &str) -> AxResult<(File, usize)> {
        crate::println!("images::fs::open_image_file: before open path={}", file_name);
        ...
        crate::println!("images::fs::open_image_file: after open path={}", file_name);
        let file_size = file
            .metadata()
            .map_err(|err| {
                ax_err_type!(
            ...
            .len() as usize;
        crate::println!(
            "images::fs::open_image_file: metadata ok path={} size={}",
            file_name,
            file_size
        );
        Ok((file, file_size))
...
fn x86_linux_layout_error(err: x86_linux::X86LinuxLayoutError) -> ax_errno::AxError {
    crate::println!("x86_linux_layout_error: {:?}", err);
    ax_errno::ax_err_type!(
        InvalidInput,
