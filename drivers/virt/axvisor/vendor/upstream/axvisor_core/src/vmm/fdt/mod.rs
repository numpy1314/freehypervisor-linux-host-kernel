...
pub fn handle_fdt_operations(
    ...
    if let Some(host_fdt_bytes) = host_fdt_bytes {
        ...
        validate_linux_host_guest_ram_reserved(vm_create_config, host_fdt_bytes)?;
        ...
        #[cfg(axvisor_host_riscv64)]
        {
            return Err(ax_err_type!(
                InvalidData,
                "Linux-host RISC-V requires host_fdt_path for guest DTB and passthrough validation"
            ));
        }
