...
fn configured_passthrough_paths(crate_config: &AxVMCrateConfig) -> Vec<String> {
    crate_config
        .devices
        .passthrough_devices
        .iter()
...
#[cfg(target_arch = "riscv64")]
fn is_riscv_required_node(node_path: &str) -> bool {
    node_path == "/"
        || node_path == "/cpus"
        || node_path.starts_with("/cpus/")
        || node_path == "/chosen"
        || node_path == "/soc"
        || node_path.contains("plic@")
        || node_path.contains("interrupt-controller")
}

#[cfg(target_arch = "riscv64")]
fn should_include_sanitized_riscv_node(
    node: &Node,
    node_path: &str,
    passthrough_device_names: &[String],
) -> bool {
    if node.name().starts_with("memory") || node_path == "/reserved-memory" {
        return false;
    }

    if is_riscv_required_node(node_path) {
        return true;
    }

    if passthrough_device_names.contains(&node_path.to_string()) {
        return true;
    }

    is_descendant_of_passthrough_device(node_path, node.level, passthrough_device_names)
        || is_ancestor_of_passthrough_device(node_path, passthrough_device_names)
}

#[cfg(target_arch = "riscv64")]
fn add_config_memory_node(
    crate_config: &AxVMCrateConfig,
...
#[cfg(target_arch = "riscv64")]
pub(crate) fn sanitize_provided_guest_fdt(
    fdt: &Fdt,
    crate_config: &AxVMCrateConfig,
) -> AxResult<Vec<u8>> {
    let passthrough_device_names = configured_passthrough_paths(crate_config);
    let all_nodes: Vec<Node> = fdt.all_nodes().collect();
    let all_paths = super::build_all_node_paths(&all_nodes);
    ...
    for (index, node) in all_nodes.iter().enumerate() {
        let node_path = &all_paths[index];
        if !should_include_sanitized_riscv_node(node, node_path, &passthrough_device_names) {
            continue;
        }
    ...
    if !has_chosen {
        ...
        let bootargs = effective_bootargs(crate_config, "");
        new_fdt
            .property_string("bootargs", &bootargs)
            .map_err(fdt_write_err)?;
        new_fdt.end_node(chosen).map_err(fdt_write_err)?;
    }

    let memory_node = new_fdt.begin_node("memory").map_err(fdt_write_err)?;
    add_config_memory_node(crate_config, &mut new_fdt)?;
...
pub(crate) fn patch_guest_fdt_for_runtime(
    ...
    if !has_chosen {
        if let Some(host_fdt) = host_fdt {
            if let Some(chosen_node) = host_fdt.find_nodes("/chosen").next() {
                let chosen = new_fdt.begin_node("chosen").map_err(fdt_write_err)?;
