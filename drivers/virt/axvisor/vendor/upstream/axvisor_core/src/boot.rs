...
pub fn run() {
    print_logo();
    crate::println!("boot::run stage=after_logo");

    info!("Starting virtualization...");
    info!("Hardware support: {:?}", axvm::has_hardware_support());
    crate::println!("boot::run stage=before_ensure_hardware_support");
    ensure_hardware_support();
    crate::println!("boot::run stage=after_ensure_hardware_support");
    crate::println!("boot::run stage=before_enable_virtualization");
    if let Err(err) = enable_virtualization_on_all_cores() {
        warn!("Failed to enable hardware virtualization: {:?}", err);
        return;
    }
    crate::println!("boot::run stage=after_enable_virtualization");

    crate::println!("boot::run stage=before_vmm_init");
    vmm::init();
    crate::println!("boot::run stage=after_vmm_init");

    info!("[OK] VMM initialized; waiting for shell commands to start guests");
    crate::println!("boot::run stage=before_shell");

    #[cfg(feature = "shell")]
    {
        info!("boot: entering shell::console_init");
        crate::shell::console_init();
        info!("boot: shell::console_init returned unexpectedly");
    }
}
...
fn enable_virtualization_on_all_cores() -> AxResult {
    ...
    for cpu_id in 0..cpu_count {
        task::spawn_task(
            ...
            move || {
                crate::println!("boot::enable core={cpu_id} stage=entry");
                host::init_percpu();
                crate::println!("boot::enable core={cpu_id} stage=after_host_init_percpu");
                ...
                crate::println!("boot::enable core={cpu_id} stage=after_timer_init");
                ...
                if let Err(err) = percpu.init(cpu_id) {
                    warn!("Core {cpu_id} failed to initialize per-CPU state: {:?}", err);
                    crate::println!("boot::enable core={cpu_id} stage=percpu_init_failed err={err:?}");
                    FAILURE.store(err.code() as usize + 1, Ordering::Release);
                    return;
                }
                crate::println!("boot::enable core={cpu_id} stage=after_percpu_init");
                if let Err(err) = percpu.hardware_enable() {
                    warn!("Core {cpu_id} failed to enable virtualization: {:?}", err);
                    crate::println!("boot::enable core={cpu_id} stage=hardware_enable_failed err={err:?}");
                    FAILURE.store(err.code() as usize + 1, Ordering::Release);
                    return;
                }
                crate::println!("boot::enable core={cpu_id} stage=after_hardware_enable");
