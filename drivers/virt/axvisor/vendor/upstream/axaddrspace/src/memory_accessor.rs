...
use crate::{AxMmHal, GuestPhysAddr};

/// A stateful accessor to the memory space of a guest
pub trait GuestMemoryAccessor: AxMmHal {
    ...
    fn read_obj<V: Copy>(&self, guest_addr: GuestPhysAddr) -> AxResult<V> {
        ...
        unsafe {
            let ptr = Self::phys_to_virt(host_addr).as_ptr() as *const V;
    ...
    fn write_obj<V: Copy>(&self, guest_addr: GuestPhysAddr, val: V) -> AxResult<()> {
        ...
        unsafe {
            let ptr = Self::phys_to_virt(host_addr).as_mut_ptr() as *mut V;
    ...
    fn read_buffer(&self, guest_addr: GuestPhysAddr, buffer: &mut [u8]) -> AxResult<()> {
        ...
        if accessible_size >= buffer.len() {
            ...
            unsafe {
                let src_ptr = Self::phys_to_virt(host_addr).as_ptr();
        ...
        while !remaining_buffer.is_empty() {
            ...
            unsafe {
                let src_ptr = Self::phys_to_virt(current_host_addr).as_ptr();
    ...
    fn write_buffer(&self, guest_addr: GuestPhysAddr, buffer: &[u8]) -> AxResult<()> {
        ...
        if accessible_size >= buffer.len() {
            // Simple case: entire buffer fits within accessible region
            unsafe {
                let dst_ptr = Self::phys_to_virt(host_addr).as_mut_ptr();
        ...
        while !remaining_buffer.is_empty() {
            ...
            unsafe {
                let dst_ptr = Self::phys_to_virt(current_host_addr).as_mut_ptr();
