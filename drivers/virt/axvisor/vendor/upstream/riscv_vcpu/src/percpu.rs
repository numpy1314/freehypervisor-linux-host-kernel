...
static RISCV_HYP_ENABLED: AtomicBool = AtomicBool::new(false);
...
impl AxArchPerCpu for RISCVPerCpu {
    ...
    fn is_enabled(&self) -> bool {
        RISCV_HYP_ENABLED.load(Ordering::Acquire)
    }

    fn hardware_enable(&mut self) -> AxResult<()> {
        if has_hardware_support() {
            RISCV_HYP_ENABLED.store(true, Ordering::Release);
            Ok(())
        } else {
    ...
    fn hardware_disable(&mut self) -> AxResult<()> {
        RISCV_HYP_ENABLED.store(false, Ordering::Release);
        Ok(())
    }
}

/// Initialize (H)S-level CSRs to a reasonable state.
pub(crate) fn setup_hypervisor_csrs() {
    unsafe {
