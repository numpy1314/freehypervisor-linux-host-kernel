    ...
    unsafe {
        cfg_if::cfg_if! {
            if #[cfg(target_arch = "x86_64")] {
                #[cfg(axvisor_host_x86_64)]
                {
                    tp = _percpu_base_ptr(axvisor_linux_host_current_cpu_id()) as usize;
                }
                #[cfg(all(target_os = "linux", not(axvisor_host_x86_64)))]
                {
                    tp = GS;
                }
                #[cfg(all(target_os = "none", not(axvisor_host_x86_64)))]
                {
                    tp = x86::msr::rdmsr(x86::msr::IA32_GS_BASE) as usize
                }
                #[cfg(all(not(axvisor_host_x86_64), not(target_os = "linux"), not(target_os = "none")))]
                {
                    unimplemented!()
                }
...
unsafe extern "C" {
    fn _percpu_base_ptr(cpu_idx: usize) -> *mut u8;
