...
macro_rules! read_csr {
    ($csr_number:expr) => {
        ...
        unsafe fn _read() -> usize {
            match () {
                #[cfg(any(riscv, target_arch = "riscv32", target_arch = "riscv64"))]
                () => {
                    let r: usize;
                ...
                #[cfg(not(any(riscv, target_arch = "riscv32", target_arch = "riscv64")))]
                () => unimplemented!(),
            }
        }
    };
...
macro_rules! read_csr_as {
    ...
    ($register:ident, $csr_number:expr, $asm_fn: ident) => {
        $crate::read_csr!($csr_number, $asm_fn);
...
macro_rules! read_csr_as_usize_rv32 {
    ($csr_number:expr, $asm_fn: ident) => {
        $crate::read_csr_rv32!($csr_number, $asm_fn);
...
macro_rules! write_csr_as_usize {
    ...
    ($csr_number:expr, $asm_fn: ident) => {
        $crate::write_csr!($csr_number, $asm_fn);
...
macro_rules! write_csr_as_usize_rv32 {
    ($csr_number:expr, $asm_fn: ident) => {
        $crate::write_csr_rv32!($csr_number, $asm_fn);
